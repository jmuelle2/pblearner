class CommentsController < ApplicationController
  def index
    ...
    @comments = @q.result(:distinct => true).includes(:response).page(params[:page]).per(10)
  ...
  def create
    ...
    @comment.response_id = params[:response_id]
    @comment.content = params[:content]
    @comment.student_id = params[:student_id]
    @comment.instructor_id = params[:instructor_id]
